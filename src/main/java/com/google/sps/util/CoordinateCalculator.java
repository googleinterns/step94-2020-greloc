...
public final class CoordinateCalculator {
  ...
  /**
   * Returns a CompositeFilter to query any entities whose Latitude lies within (latitude +
   * kilometers) and (latitude - kilometers)
   *
   * @param kilometers: Distance, in kilometers, that each bound should be from the center point of
   *     the bounding box
  ...
  public static CompositeFilter createLatitudeBoundFilter(double kilometers, double latitude) {
    ...
    return createCoordinateBoundFilter(
        CoordinateType.LATITUDE, latBounds.getUpperBound(), latBounds.getLowerBound());
  }

  /**
   * Returns a CompositeFilter to query any entities whose Longitude lies within (longitude +
   * kilometers) and (longitude - kilometers)
   *
   * @param kilometers: Distance, in kilometers, that each bound should be from the center point of
   *     the bounding box
   * @param latitude: the latitude of the center point of the bounding box
   * @param longitude: the longitude of the center point of the bounding box
   */
  public static CompositeFilter createLongitudeBoundFilter(
      double kilometers, double latitude, double longitude) {
    Bounds longBounds = calculateLongitudeBounds(kilometers, latitude, longitude);
    return createCoordinateBoundFilter(
        CoordinateType.LONGITUDE, longBounds.getUpperBound(), longBounds.getLowerBound());
  }

  /**
   * Returns a CompositeFilter to query any entities whose CoordinateType lies within a given upper
   * and lower bound
   *
   ...
   */
  public static CompositeFilter createCoordinateBoundFilter(
      CoordinateType coordinateType, double upperBound, double lowerBound) {
    FilterPredicate upperBoundFilter =
        new Query.FilterPredicate(
            coordinateType.value, FilterOperator.LESS_THAN_OR_EQUAL, upperBound);
    FilterPredicate lowerBoundFilter =
        new Query.FilterPredicate(
            coordinateType.value, FilterOperator.GREATER_THAN_OR_EQUAL, lowerBound);
   ...
   */
  public static List<Entity> filterOutOfRangeLatitudeEntities(
      double kilometers, double baseLatitude, double baseLongitude, List<Entity> entities) {
  ...
  private static void printDebug(
    ...
    System.out.println(
        "Upper Bound coordinates: ("
            + latBounds.getUpperBound()
            + ", "
            + longBounds.getUpperBound()
            + ")");
    System.out.println(
        "Lower Bound coordinates: ("
            + latBounds.getLowerBound()
            + ", "
            + longBounds.getLowerBound()
            + ")");

    System.out.println();

    System.out.println(
        "listingLatitude: " + listingLat + " <= " + latBounds.getUpperBound() + " Upper Lat Bound");
    System.out.println(
        "listingLatitude: " + listingLat + " >= " + latBounds.getLowerBound() + " Lower Lat Bound");
   ...
   *
   * @param kilometers: Distance to convert, in kilometers
   * @param baseLat: The latitude coordinate pair of the longitude to which this offset will be
   *     added/subtracted
   */
  ...
  private static Bounds calculateLongitudeBounds(
      double kilometers, double latitude, double longitude) {
