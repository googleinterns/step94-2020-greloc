...
public final class QueryHelperTest {
  ...
  public void testDateRangeReturnsInBounds() throws Exception {
    ...
    long oneDayAfterEleventh = Instant.ofEpochMilli(JULY_ELEVENTH * 1000)
      .plus(1 , ChronoUnit.DAYS)
      .getEpochSecond();

    long oneDayBeforeTwentieth = Instant.ofEpochMilli(JULY_TWENTY_FIRST * 1000)
      .minus(1 , ChronoUnit.DAYS)
      .getEpochSecond();

    List<Entity> results = QueryHelper.filterOutOfDateRangeListings(listings, oneDayAfterEleventh, oneDayBeforeTwentieth);
    Assert.assertEquals(1, results.size());
  ...
  public void testDateRangeReturnsInBoundsOnExactDates() throws Exception {
    ...
    List<Entity> results = QueryHelper.filterOutOfDateRangeListings(listings, JULY_ELEVENTH, JULY_TWENTY_FIRST);
    Assert.assertEquals(1, results.size());
  ...
  public void testDateRangeDoesNotReturnOutOfLowerBound() throws Exception {
    ...
    long oneDayBeforeEleventh = Instant.ofEpochMilli(JULY_ELEVENTH * 1000)
      .minus(1 , ChronoUnit.DAYS)
      .getEpochSecond();


    List<Entity> results = QueryHelper.filterOutOfDateRangeListings(listings, oneDayBeforeEleventh, JULY_TWENTY_FIRST);
    Assert.assertEquals(0, results.size());
  }
  ...
  public void dateRangeDoesNotReturnOutOfUpperBound() throws Exception {
    ...
    long oneDayAfterTwentyFirst = Instant.ofEpochMilli(JULY_TWENTY_FIRST * 1000)
      .plus(1 , ChronoUnit.DAYS)
      .getEpochSecond();
    
    List<Entity> results = QueryHelper.filterOutOfDateRangeListings(listings, JULY_ELEVENTH, oneDayAfterTwentyFirst);
    Assert.assertEquals(0, results.size());
  }
  ...
  public void dateRangeThrowsExceptionWithBadRangeParameters() throws Exception {
    ...
    long julyTwentySecond = Instant.ofEpochMilli(JULY_TWENTY_FIRST * 1000)
      .plus(1 , ChronoUnit.DAYS)
      .getEpochSecond();
